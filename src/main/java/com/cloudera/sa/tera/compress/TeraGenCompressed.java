...
public class TeraGenCompressed extends Configured implements Tool
{
	...
	static class RandomGenerator
	{
		...
		{ 0L, 4160749568L, 4026531840L, 3892314112L, 3758096384L, 3623878656L, 3489660928L, 3355443200L, 3221225472L, 3087007744L, 2952790016L, 2818572288L, 2684354560L, 2550136832L, 2415919104L,
				2281701376L, 2147483648L, 2013265920L, 1879048192L, 1744830464L, 1610612736L, 1476395008L, 1342177280L, 1207959552L, 1073741824L, 939524096L, 805306368L, 671088640L, 536870912L,
	...
	public static class SortGenMapper extends MapReduceBase implements Mapper<LongWritable, NullWritable, Text, Text>
	{
		...
		public void map(LongWritable row, NullWritable ignored, OutputCollector<Text, Text> output, Reporter reporter) throws IOException
		{
			...
			reporter.getCounter("Mapper", "UnCompressed Writes").increment(key.getLength() + value.getLength() + 2);
	...
	public int run(String[] args) throws IOException
	{
		...
		job.setJarByClass(TeraGenCompressed.class);
		...
		if (compressionType.equals("snappy"))
		{
		...
		} else if (compressionType.equals("none"))
		{
			FileOutputFormat.setOutputPath(job, new Path(args[1]));
			job.setOutputFormat(TeraOutputFormat.class);
		} else
		{
			System.out.println("Compression has to be one of the following:");
	...
	public static void main(String[] args) throws Exception
	{
		if (args.length < 3)
		{
			System.out.println("TeraGen - Compressed");
			System.out.println("");
			System.out.println("Parameters:");
			System.out.println("{number of records} {output file} {compression code (none, snappy, gzip)}");
			System.out.println("");
		...
		int res = ToolRunner.run(new JobConf(), new TeraGenCompressed(), args);
		System.exit(res);
	}
}