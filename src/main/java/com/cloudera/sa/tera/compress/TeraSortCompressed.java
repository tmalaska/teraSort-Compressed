...
public class TeraSortCompressed extends Configured implements Tool
{
	private static final Log LOG = LogFactory.getLog(TeraSortCompressed.class);
	...
	static class TotalOrderPartitioner implements Partitioner<Text, Text>
	{
		...
		public void configure(JobConf job)
		{
			try
			{
				...
				Path partFile = new Path(TeraInputCompressedFormat.PARTITION_FILENAME);
	...
	public int run(String[] args) throws Exception
	{
		...
		job.setJarByClass(TeraSortCompressed.class);
		...
		// partition file
		Path partitionFile = new Path(inputDir, TeraInputCompressedFormat.PARTITION_FILENAME);
		URI partitionUri = new URI(partitionFile.toString() + "#" + TeraInputCompressedFormat.PARTITION_FILENAME);
		TeraInputCompressedFormat.writePartitionFile(job, partitionFile);
	...
	public static void main(String[] args) throws Exception
	{
		if (args.length < 3)
		{
			System.out.println("TeraSort - Compressed");
			System.out.println("");
		...
		int res = ToolRunner.run(new JobConf(), new TeraSortCompressed(), args);
		System.exit(res);
	}
